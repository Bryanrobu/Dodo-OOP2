 ...
 */
public abstract class Dodo extends MovableActor
{
    ...
    public Egg getEggAhead() {
        Egg egg = getActorAhead( myDirection, Egg.class );
        if ( egg != null ){;
            return egg;
        } else {
            showError( "no egg ahead" );
            Greenfoot.stop();
            return null;
        }            
    }

    public Nest getNestAhead() {
        Nest nest = getActorAhead( myDirection, Nest.class );
        if ( nest != null ){;
            return nest;
        } else {
            showError( "no nest ahead" );
            Greenfoot.stop();
            return null;
        }            
    }
